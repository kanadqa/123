...
const LAYOUT_KEY = "budget.layout.v1";
...
const viewCopy = {
  dashboard: {
    title: "Обзор",
    subtitle: "Сводка доходов, расходов и аналитика по категориям.",
  },
  transactions: {
    title: "Операции",
    subtitle: "Добавляйте операции и управляйте историей.",
  },
  categories: {
    title: "Категории",
    subtitle: "Создавайте категории и подкатегории под ваши нужды.",
  },
  settings: {
    title: "Настройки",
    subtitle: "Выберите наиболее удобный вариант интерфейса.",
  },
};
...
const ensureSubcategory = (list) => {
  if (!Array.isArray(list) || list.length === 0) {
    return [DEFAULT_SUBCATEGORY];
  }
  if (!list.includes(DEFAULT_SUBCATEGORY)) {
    return [DEFAULT_SUBCATEGORY, ...list];
  }
  return list;
};
...
const updateSubcategoryOptions = (categoryName) => {
  ...
  const subs = ensureSubcategory(categories[categoryName]);
  subs.forEach((sub) => {
    const option = document.createElement("option");
    option.value = sub;
...
const setView = (view) => {
  ...
  viewTitle.textContent = viewCopy[view]?.title || "Раздел";
  viewSubtitle.textContent = viewCopy[view]?.subtitle || "";
};
...
const storedLayout = localStorage.getItem(LAYOUT_KEY) || "comfort";
